...
function run() {
  ...
  /* _divItem global variable */
  const initialState = document.getElementById(idState).value;
  const bytes = AES.decrypt(initialState, keyState);
  const stateDescryption = bytes.toString(enc);
  const newState = JSON.parse(stateDescryption);
  ...
  context.store = configureStore(newState, { history });
